...
/// OptProxyProposalMsg contains an optimistic proxy block data and sync info.
/// Used for 1-message-delay block time in proxy consensus.
#[derive(Clone, Debug, Deserialize, Eq, PartialEq, Serialize)]
pub struct OptProxyProposalMsg {
    block_data: OptProxyBlockData,
    sync_info: ProxySyncInfo,
}

impl OptProxyProposalMsg {
    pub fn new(block_data: OptProxyBlockData, sync_info: ProxySyncInfo) -> Self {
        Self {
            block_data,
            sync_info,
        }
    ...
    pub fn block_data(&self) -> &OptProxyBlockData {
        &self.block_data
    }

    pub fn take_block_data(self) -> OptProxyBlockData {
        self.block_data
    }
    ...
    pub fn verify_well_formed(&self) -> Result<()> {
        self.block_data
            .verify_well_formed()
            .context("Failed to verify OptProxyBlockData")?;
        ensure!(
    ...
    pub fn verify(
        ...
        // Verify primary QC if attached
        if let Some(primary_qc) = self.block_data.primary_qc() {
            // Note: Primary QC may be verified against the full validator set,
            // but for now we verify with proxy validator set
            primary_qc
                .verify(validator)
                .context("Failed to verify attached primary QC")?;
        }
...
pub struct OrderedProxyBlocksMsg {
    ...
    primary_round: Round,
    /// The primary QC that "cut" these proxy blocks.
    /// QC.round == primary_round - 1
    primary_qc: QuorumCert,
}

impl OrderedProxyBlocksMsg {
    pub fn new(proxy_blocks: Vec<Block>, primary_round: Round, primary_qc: QuorumCert) -> Self {
        Self {
            proxy_blocks,
            primary_round,
            primary_qc,
    ...
    pub fn epoch(&self) -> u64 {
        self.primary_qc.certified_block().epoch()
    }
    ...
    pub fn verify(&self, proxy_verifier: &ValidatorVerifier) -> Result<()> {
        ...
        // Verify primary QC round matches primary_round - 1
            ...
            self.primary_qc.certified_block().round() == self.primary_round.saturating_sub(1),
            "Primary QC round {} doesn't match primary_round - 1 = {}",
            self.primary_qc.certified_block().round(),
        ...
        // Verify the last block has the primary QC attached
        let last_block = self.proxy_blocks.last().unwrap();
        ensure!(
            last_block.block_data().primary_qc().is_some(),
            "Last proxy block must have primary QC attached"
        );
        ensure!(
            last_block.block_data().primary_qc().unwrap().certified_block()
                == self.primary_qc.certified_block(),
            "Last proxy block's primary QC doesn't match message's primary QC"
        );
...
impl Display for OrderedProxyBlocksMsg {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        write!(
            f,
            "OrderedProxyBlocksMsg: [primary_round: {}, num_blocks: {}, primary_qc_round: {}]",
            self.primary_round,
...
mod tests {
    ...
    fn test_ordered_proxy_blocks_msg_creation() {
        ...
        let msg = OrderedProxyBlocksMsg::new(vec![], 1, qc);
