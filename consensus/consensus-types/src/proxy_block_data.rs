...
/// - `primary_qc`: Attached when the primary QC for `primary_round - 1` is available
...
pub enum OptProxyBlockBody {
    V0 {
        ...
        primary_round: Round,
        /// Primary QC attached when QC.round == primary_round - 1
        /// This QC "cuts" the proxy blocks for the previous primary round
...
impl OptProxyBlockData {
    ...
    pub fn verify_well_formed(&self) -> anyhow::Result<()> {
        ...
        // Primary QC attachment validation
        if let Some(ref primary_qc) = self.primary_qc() {
            ensure!(
                primary_qc.certified_block().round() == self.primary_round().saturating_sub(1),
                "Primary QC round {} must equal primary_round - 1 = {}",
    ...
    /// Returns true if this proxy block has a primary QC attached (cutting point).
    pub fn has_primary_qc(&self) -> bool {
        self.primary_qc().is_some()
    }
}
...
impl OptProxyBlockBody {
    ...
    pub fn primary_qc(&self) -> Option<&QuorumCert> {
        match self {
            OptProxyBlockBody::V0 { primary_qc, .. } => primary_qc.as_ref(),
        }
    }
}
...
impl Display for OptProxyBlockData {
    fn fmt(&self, f: &mut Formatter) -> std::fmt::Result {
        write!(
            f,
            "[proxy author: {}, epoch: {}, round: {:02}, primary_round: {:02}, parent_id: {}, timestamp: {}, has_primary_qc: {}]",
            self.author(),
            self.epoch(),
            self.round(),
            self.primary_round(),
...
mod tests {
    ...
    fn test_opt_proxy_block_data_creation() {
        ...
        assert!(!block.has_primary_qc());
    }

    #[test]
    fn test_opt_proxy_block_data_with_primary_qc() {
        let epoch = 1;
        let grandparent_qc = make_qc(epoch, 1, 1000);
        let parent = make_block_info(epoch, 2, 2000);
        ...
        let block = OptProxyBlockData::new(
            ...
            parent,
            grandparent_qc,
            1, // primary_round = 1, so primary_qc should be for round 0
            Some(primary_qc),
        );

        assert!(block.has_primary_qc());
        assert_eq!(block.primary_qc().unwrap().certified_block().round(), 0);
    }
}
